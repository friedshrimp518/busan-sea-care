...
document.querySelectorAll('.map-pin').forEach((pin) => pin.addEventListener('click', () => { placeName.textContent = pin.dataset.name; placeKind.textContent = pin.dataset.kind; placeDistance.textContent = fullScreenCopy[currentLanguage].nearbyOpen; }));
...
const fullScreenCopy = {
  en: { marineHeader: 'Marine Information', facilityHeader: 'Nearby Facilities', analysis: 'AI ANALYSIS', analysisTitle: 'Good conditions for coastal activities', analysisText: 'Low waves and good visibility make this a generally safe time for beach walks and swimming near lifeguard zones. Stay aware of changing currents.', caution: ['Use sunscreen and drink water regularly.', 'Keep away from rocks as the tide rises.', 'Do not enter the water if a warning flag is raised.'], cancel: 'Cancel', nearbyOpen: 'Nearby - Open now', analysisLink: 'Open analysis >', agency: [['Coast Guard', 'Marine emergencies'], ['Police', 'Safety threats and crime'], ['Fire & Ambulance', 'Fire and medical emergencies']] },
  ko: { marineHeader: '해양 정보', facilityHeader: '주변 편의시설', analysis: 'AI 분석', analysisTitle: '해안 활동에 좋은 상태입니다', analysisText: '파도가 낮고 시야가 좋아 안전요원이 있는 구역에서 산책과 수영을 하기에 비교적 안전합니다. 변화하는 해류에 유의하세요.', caution: ['자외선 차단제를 바르고 물을 충분히 마시세요.', '밀물이 들 때는 바위 주변에서 떨어지세요.', '경고 깃발이 올라가면 물에 들어가지 마세요.'], cancel: '취소', nearbyOpen: '주변 · 운영 중', analysisLink: '분석 보기 >', agency: [['해양경찰', '해양 응급상황'], ['경찰', '범죄 및 안전 위협'], ['소방·구급', '화재 및 의료 응급상황']] },
  zh: { marineHeader: '海洋信息', facilityHeader: '附近设施', analysis: 'AI 分析', analysisTitle: '适合进行海岸活动的状况', analysisText: '海浪较低且能见度良好，在有救生员的区域散步和游泳通常较安全。请注意不断变化的海流。', caution: ['请使用防晒霜并定期补充水分。', '涨潮时请远离岩石区域。', '如升起警告旗，请勿下水。'], cancel: '取消', nearbyOpen: '附近 · 正在营业', analysisLink: '查看分析 >', agency: [['海岸警卫队', '海上紧急情况'], ['警察', '安全威胁和犯罪'], ['消防与救护', '火灾和医疗紧急情况']] },
  ja: { marineHeader: '海洋情報', facilityHeader: '周辺施設', analysis: 'AI 分析', analysisTitle: '海岸での活動に適した状況です', analysisText: '波が低く視界も良好なため、ライフガードのいる区域での散歩や遊泳は比較的安全です。変化する潮流に注意してください。', caution: ['日焼け止めを使い、こまめに水分を補給してください。', '潮が満ちてくる際は岩場から離れてください。', '警告旗が上がっている場合は入水しないでください。'], cancel: 'キャンセル', nearbyOpen: '近く · 営業中', analysisLink: '分析を見る >', agency: [['海上保安庁', '海上緊急事態'], ['警察', '犯罪・安全上の脅威'], ['消防・救急', '火災・医療緊急事態']] },
  es: { marineHeader: 'Información marina', facilityHeader: 'Servicios cercanos', analysis: 'ANÁLISIS DE IA', analysisTitle: 'Buenas condiciones para actividades costeras', analysisText: 'Las olas bajas y la buena visibilidad hacen que caminar y nadar cerca de zonas con socorristas sea relativamente seguro. Presta atención a los cambios de corrientes.', caution: ['Usa protector solar y bebe agua con regularidad.', 'Mantente alejado de las rocas cuando suba la marea.', 'No entres al agua si hay una bandera de advertencia.'], cancel: 'Cancelar', nearbyOpen: 'Cerca · Abierto ahora', analysisLink: 'Ver análisis >', agency: [['Guardia Costera', 'Emergencias marítimas'], ['Policía', 'Amenazas y delitos'], ['Bomberos y ambulancia', 'Emergencias médicas y de incendio']] }
};
const applyFullScreenCopy = () => {
  ...
  document.querySelector('.ai-card h2').textContent = copy.analysisTitle;
  ...
  document.querySelectorAll('.agency-button').forEach((button, index) => {
    ...
    label.replaceChildren(document.createTextNode(name), document.createElement('br'), Object.assign(document.createElement('small'), { textContent: detail }));
  });
  if (placeName.textContent === 'CU Haeundae Beach') placeDistance.textContent = copy.nearbyOpen;
};
